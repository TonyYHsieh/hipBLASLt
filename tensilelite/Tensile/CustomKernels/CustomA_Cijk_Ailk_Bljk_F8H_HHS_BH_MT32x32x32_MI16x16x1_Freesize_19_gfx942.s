...
s_cbranch_scc0 label_NoBranch_Y2Z2VM52PPBDPGB1_0   // Only branch on scc1
...
label_NoBranch_Y2Z2VM52PPBDPGB1_0:
s_waitcnt vmcnt(0)                                 // 8wait for global read

/* local write a */
v_cvt_pk_f32_fp8 v[24:25], v[vgprG2LA+1+0] src0_sel:WORD_0 // convert to F32
v_cvt_f16_f32 v[vgprG2LA+0+0], v24 dst_sel:WORD_0  // Convert to FP16
v_cvt_f16_f32 v[vgprG2LA+0+0], v25 dst_sel:WORD_1  // Convert to FP16
v_cvt_pk_f32_fp8 v[24:25], v[vgprG2LA+1+0] src0_sel:WORD_1 // convert to F32
v_cvt_f16_f32 v[vgprG2LA+0+1], v24 dst_sel:WORD_0  // Convert to FP16
v_cvt_f16_f32 v[vgprG2LA+0+1], v25 dst_sel:WORD_1  // Convert to FP16
...
label_Summation_End_62ABMZLZ6WS15RQQ_0:
/* endSummation: add vgpr [0...22) to pool */
.set sgprWGM, UNDEF
...
s_waitcnt lgkmcnt(0)                               // wait for 16 bytes of kern args.
s_cmp_eq_u32 s[sgprGSU], 1                         // GSU == 1 ?
s_cbranch_scc0 label_NoBranch_ZV489OUCV6KZWPRR_0   // Only branch on scc1
...
label_NoBranch_ZV489OUCV6KZWPRR_0:
...
v_cndmask_b32 v6, v23, v6, s[56:57]                // LDC clip if OOB. offset
buffer_load_short_d16 v8, v6, s[sgprSrdC:sgprSrdC+3], 0 offen offset:0 // load C
v_add_lshl_u32 v6, v3, v0, 0x2                     // scaleToBpe: accumulate d0 lower and *= bpe into Cin addr
v_cndmask_b32 v6, v23, v6, s[56:57]                // LDD clip if OOB. offset
v_add_lshl_u32 v7, v3, v0, 0x1                     // scaleToBpe: accumulate d0 lower and *= bpe into Cin addr
v_cndmask_b32 v7, v23, v7, s[56:57]                // LDTD clip if OOB. offset
/* (d1,vc1,d0,vc0)=(0,0,0,1) */
v_add_co_u32 v4, vcc, v0, 1                        // coord0.1: coord0 += d0*sg0*VW + vc0
v_cmp_lt_u32 s[52:53], v4, s[sgprSizeI]            // coord0 < size0
v_cmp_lt_u32 s[56:57], v1, s[sgprSizeJ]            // coord1 < size1
...
v_cndmask_b32 v10, v23, v10, s[56:57]              // LDC clip if OOB. offset
...
v_cndmask_b32 v10, v23, v10, s[56:57]              // LDD clip if OOB. offset
v_add_lshl_u32 v11, v3, v4, 0x1                    // scaleToBpe: accumulate d0 lower and *= bpe into Cin addr
v_cndmask_b32 v11, v23, v11, s[56:57]              // LDTD clip if OOB. offset
/* (d1,vc1,d0,vc0)=(0,0,0,2) */
v_add_co_u32 v4, vcc, v0, 2                        // coord0.1: coord0 += d0*sg0*VW + vc0
v_cmp_lt_u32 s[52:53], v4, s[sgprSizeI]            // coord0 < size0
v_cmp_lt_u32 s[56:57], v1, s[sgprSizeJ]            // coord1 < size1
...
v_cndmask_b32 v14, v23, v14, s[56:57]              // LDC clip if OOB. offset
...
v_cndmask_b32 v14, v23, v14, s[56:57]              // LDD clip if OOB. offset
v_add_lshl_u32 v15, v3, v4, 0x1                    // scaleToBpe: accumulate d0 lower and *= bpe into Cin addr
v_cndmask_b32 v15, v23, v15, s[56:57]              // LDTD clip if OOB. offset
/* (d1,vc1,d0,vc0)=(0,0,0,3) */
v_add_co_u32 v4, vcc, v0, 3                        // coord0.1: coord0 += d0*sg0*VW + vc0
v_cmp_lt_u32 s[52:53], v4, s[sgprSizeI]            // coord0 < size0
v_cmp_lt_u32 s[56:57], v1, s[sgprSizeJ]            // coord1 < size1
...
v_cndmask_b32 v18, v23, v18, s[56:57]              // LDC clip if OOB. offset
...
v_cndmask_b32 v18, v23, v18, s[56:57]              // LDD clip if OOB. offset
v_add_lshl_u32 v19, v3, v4, 0x1                    // scaleToBpe: accumulate d0 lower and *= bpe into Cin addr
v_cndmask_b32 v19, v23, v19, s[56:57]              // LDTD clip if OOB. offset
...
s_addc_u32 s65, s65, s59
v_cmp_ge_i32 s[60:61], 0, s[sgprGSUSync]
v_cndmask_b32 v24, v6, v23, s[60:61]               // 1. mul 1 if 0
buffer_load_dword v28, v24, s[64:67], 0 offen offset:0, sc0 sc1 // load GSU DD
...
s_addc_u32 s65, s65, s59
v_cmp_ge_i32 s[60:61], 0, s[sgprGSUSync]
v_cndmask_b32 v24, v10, v23, s[60:61]              // 1. mul 1 if 0
buffer_load_dword v28, v24, s[64:67], 0 offen offset:0, sc0 sc1 // load GSU DD
...
s_addc_u32 s65, s65, s59
v_cmp_ge_i32 s[60:61], 0, s[sgprGSUSync]
v_cndmask_b32 v24, v14, v23, s[60:61]              // 1. mul 1 if 0
buffer_load_dword v28, v24, s[64:67], 0 offen offset:0, sc0 sc1 // load GSU DD
...
s_addc_u32 s65, s65, s59
v_cmp_ge_i32 s[60:61], 0, s[sgprGSUSync]
v_cndmask_b32 v24, v18, v23, s[60:61]              // 1. mul 1 if 0
buffer_load_dword v28, v24, s[64:67], 0 offen offset:0, sc0 sc1 // load GSU DD
