...
s_mov_b32 m0, 0x7e00                               // LDS clamp at 32256 bytes
...
s_mov_b32 s51, 96                                  // LSU offset: stride = MT1(96) + PAD1(0)
...
s_mul_i32 s51, s51, 64                             // Each wave loads continuous lsp(16)*nrp(4) columns
...
v_mov_b32 v8, 24                                   // Global Read Wave: add back to cloumn index
...
s_mul_i32 s87, s87, s[sgprNumWorkGroups0]          // (wg1 % WGM)*nwg0
s_add_u32 s87, s87, s[sgprWorkGroup0]              // wgSerial = wg0 + (wg1 % WGM)*nwg0
v_cvt_f32_u32 v6, s[sgprWGM]                       // WGM
v_rcp_iflag_f32 v6, v6                             // WGM
...
s_sub_u32 s88, s[sgprNumWorkGroups1], s85          // WorkGroup1=remainder
s_cmp_eq_u32 s88, 0                                // remainder == 0 ?
s_cmov_b32 s88, s[sgprWGM]                         // remainder = WGM if remainder == 0
s_cmp_ge_u32 s86, s84                              // blockId >= numFullBlocks ?
s_cselect_b32 s84, s88, s[sgprWGM]
v_cvt_f32_u32 v6, s84                              // s[sgprWorkGroup0] = s87 / s84
v_rcp_iflag_f32 v6, v6                             // s[sgprWorkGroup0] = s87 / s84
v_cvt_f32_u32 v7, s87                              // s[sgprWorkGroup0] = s87 / s84
v_mul_f32 v6, v6, v7                               // s[sgprWorkGroup0] = s87 / s84
v_cvt_u32_f32 v6, v6                               // s[sgprWorkGroup0] = s87 / s84
v_mul_u32_u24 v7, v6, s84                          // s[sgprWorkGroup0] = s87 / s84
v_sub_u32 v7, s87, v7                              // s[sgprWorkGroup0] = s87 / s84
v_cmpx_eq_u32 exec, v7, s84                        // s[sgprWorkGroup0] = s87 / s84
v_add_u32 v6, 1, v6                                // s[sgprWorkGroup0] = s87 / s84
v_mov_b32 v7, 0                                    // s[sgprWorkGroup1] = s87 % s84
s_mov_b64 exec, -1                                 // s[sgprWorkGroup0] = s87 / s84
...
s_mul_hi_u32 s87, s[sgprWorkGroup0], 16            // WorkGroup[01] * MT
s_mul_i32 s86, s[sgprWorkGroup0], 16               // WorkGroup[01] * MT
s_mul_hi_u32 s85, 128, s[sgprGSUSumIdx]            // gsuOffset = DepthU*bpeGR*GSUSumIdx
s_mul_i32 s84, 128, s[sgprGSUSumIdx]               // gsuOffset = DepthU*bpeGR*GSUSumIdx
s_mul_hi_u32 s85, s84, s[sgprStrideAL]             // tlu=1, scaled unroll-offset by stride
s_mul_i32 s84, s84, s[sgprStrideAL]                // tlu=1, scaled unroll-offset by stride
...
/* max read offset = size[n] * stride[n-1] */
s_mul_hi_u32 s87, s[sgprWorkGroup1], 96            // WorkGroup[01] * MT
s_mul_i32 s86, s[sgprWorkGroup1], 96               // WorkGroup[01] * MT
...
s_mul_i32 s84, s[sgprGSU], DepthU*BpeAGR
s_mul_i32 s[sgprGlobalReadIncsA+0], s84, s[sgprStrideAL] // incrA unrollIdx)

/* global read addresses: increments b */
s_mul_i32 s84, s[sgprGSU], DepthU*BpeBGR
s_mov_b32 s[sgprGlobalReadIncsB+0], s84            // incrB (unrollIdx)
...
s_add_u32 s84, 1, s[sgprLoopCounterL]              // tmp<-numIterMyWg+
s_cmp_lt_u32 s[sgprGSUSumIdx], s[sgprGSUSumIdx+1]  // gsuSumIdx < numIterPerWgRemainder
s_cmov_b32 s[sgprLoopCounterL], s84                // numIterMyWg++ if needed
label_GSU_1:
s_mov_b32 s[sgprOrigLoopCounter], s[sgprLoopCounterL] // copy loop counter
s_mov_b32 s84, 0x4                                 // init staggerU
label_beginStaggerUIter:
s_lshl_b32 s85, s84, 1                             // shift by StaggerUStride
...
buffer_load_dwordx2 v[vgprG2LB+34:vgprG2LB+34+1], v[vgprGlobalReadOffsetB+0], s[sgprSrdB:sgprSrdB+3], s[sgprScalarGlobalReadOffsetB+7] offen offset:0, sc0 // G -> Reg 0_0_8_0
buffer_load_dwordx2 v[vgprG2LB+38:vgprG2LB+38+1], v[vgprGlobalReadOffsetB+0], s[sgprSrdB:sgprSrdB+3], s[sgprScalarGlobalReadOffsetB+8] offen offset:0, sc0 // G -> Reg 0_0_9_0
buffer_load_dwordx2 v[vgprG2LB+42:vgprG2LB+42+1], v[vgprGlobalReadOffsetB+0], s[sgprSrdB:sgprSrdB+3], s[sgprScalarGlobalReadOffsetB+9] offen offset:0, sc0 // G -> Reg 0_0_10_0
buffer_load_dwordx2 v[vgprG2LB+46:vgprG2LB+46+1], v[vgprGlobalReadOffsetB+0], s[sgprSrdB:sgprSrdB+3], s[sgprScalarGlobalReadOffsetB+10] offen offset:0, sc0 // G -> Reg 0_0_11_0
...
s_cbranch_scc0 label_NoBranch_PPUU2X1UZXT9N58P_0   // Only branch on scc1
s_getpc_b64 s[84:85]                               // addr of next instr
s_add_i32 s86, label_PrefetchGlobalLastIterEnd, 0x4 // target branch offset
...
label_NoBranch_PPUU2X1UZXT9N58P_0:
...
ds_write_b16 v[vgprLocalWriteAddrA], v[vgprG2LA+4] offset:64 // lwoA_0_0_2_0 = (0 + 0*LSCA) + (2*LSPA)(*MT0I+PAD) = 64
ds_write_b16_d16_hi v[vgprLocalWriteAddrA], v[vgprG2LA+4] offset:352 // lwoA_0_1_2_0 = (1 + 0*LSCA) + (2*LSPA)(*MT0I+PAD) = 352
ds_write_b16 v[vgprLocalWriteAddrA], v[vgprG2LA+5] offset:640 // lwoA_0_2_2_0 = (2 + 0*LSCA) + (2*LSPA)(*MT0I+PAD) = 640
ds_write_b16_d16_hi v[vgprLocalWriteAddrA], v[vgprG2LA+5] offset:928 // lwoA_0_3_2_0 = (3 + 0*LSCA) + (2*LSPA)(*MT0I+PAD) = 928
ds_write_b16 v[vgprLocalWriteAddrA], v[vgprG2LA+6] offset:96 // lwoA_0_0_3_0 = (0 + 0*LSCA) + (3*LSPA)(*MT0I+PAD) = 96
ds_write_b16_d16_hi v[vgprLocalWriteAddrA], v[vgprG2LA+6] offset:384 // lwoA_0_1_3_0 = (1 + 0*LSCA) + (3*LSPA)(*MT0I+PAD) = 384
ds_write_b16 v[vgprLocalWriteAddrA], v[vgprG2LA+7] offset:672 // lwoA_0_2_3_0 = (2 + 0*LSCA) + (3*LSPA)(*MT0I+PAD) = 672
ds_write_b16_d16_hi v[vgprLocalWriteAddrA], v[vgprG2LA+7] offset:960 // lwoA_0_3_3_0 = (3 + 0*LSCA) + (3*LSPA)(*MT0I+PAD) = 960

/* local write b */
v_cvt_pk_f32_fp8 v[128:129], v[vgprG2LB+2+0] src0_sel:WORD_0 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+0+0], v128 dst_sel:WORD_0 // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+0+0], v129 dst_sel:WORD_1 // Convert to FP16
v_cvt_pk_f32_fp8 v[128:129], v[vgprG2LB+2+0] src0_sel:WORD_1 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+0+1], v128 dst_sel:WORD_0 // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+0+1], v129 dst_sel:WORD_1 // Convert to FP16
v_cvt_pk_f32_fp8 v[128:129], v[vgprG2LB+2+1] src0_sel:WORD_0 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+0+2], v128 dst_sel:WORD_0 // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+0+2], v129 dst_sel:WORD_1 // Convert to FP16
v_cvt_pk_f32_fp8 v[128:129], v[vgprG2LB+2+1] src0_sel:WORD_1 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+0+3], v128 dst_sel:WORD_0 // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+0+3], v129 dst_sel:WORD_1 // Convert to FP16
ds_write_b128 v[vgprLocalWriteAddrB], v[vgprG2LB+0:vgprG2LB+0+3] offset:0 // lwoB_0_0_0_0 = (0*LSCB)*(MT1J+PAD) + (0*LSPB) = 0
v_cvt_pk_f32_fp8 v[128:129], v[vgprG2LB+6+0] src0_sel:WORD_0 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+4+0], v128 dst_sel:WORD_0 // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+4+0], v129 dst_sel:WORD_1 // Convert to FP16
v_cvt_pk_f32_fp8 v[128:129], v[vgprG2LB+6+0] src0_sel:WORD_1 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+4+1], v128 dst_sel:WORD_0 // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+4+1], v129 dst_sel:WORD_1 // Convert to FP16
v_cvt_pk_f32_fp8 v[128:129], v[vgprG2LB+6+1] src0_sel:WORD_0 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+4+2], v128 dst_sel:WORD_0 // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+4+2], v129 dst_sel:WORD_1 // Convert to FP16
v_cvt_pk_f32_fp8 v[128:129], v[vgprG2LB+6+1] src0_sel:WORD_1 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+4+3], v128 dst_sel:WORD_0 // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+4+3], v129 dst_sel:WORD_1 // Convert to FP16
ds_write_b128 v[vgprLocalWriteAddrB], v[vgprG2LB+4:vgprG2LB+4+3] offset:576 // lwoB_0_0_1_0 = (0*LSCB)*(MT1J+PAD) + (1*LSPB) = 576
v_cvt_pk_f32_fp8 v[128:129], v[vgprG2LB+10+0] src0_sel:WORD_0 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+8+0], v128 dst_sel:WORD_0 // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+8+0], v129 dst_sel:WORD_1 // Convert to FP16
v_cvt_pk_f32_fp8 v[128:129], v[vgprG2LB+10+0] src0_sel:WORD_1 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+8+1], v128 dst_sel:WORD_0 // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+8+1], v129 dst_sel:WORD_1 // Convert to FP16
v_cvt_pk_f32_fp8 v[128:129], v[vgprG2LB+10+1] src0_sel:WORD_0 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+8+2], v128 dst_sel:WORD_0 // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+8+2], v129 dst_sel:WORD_1 // Convert to FP16
v_cvt_pk_f32_fp8 v[128:129], v[vgprG2LB+10+1] src0_sel:WORD_1 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+8+3], v128 dst_sel:WORD_0 // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+8+3], v129 dst_sel:WORD_1 // Convert to FP16
ds_write_b128 v[vgprLocalWriteAddrB], v[vgprG2LB+8:vgprG2LB+8+3] offset:1152 // lwoB_0_0_2_0 = (0*LSCB)*(MT1J+PAD) + (2*LSPB) = 1152
v_cvt_pk_f32_fp8 v[128:129], v[vgprG2LB+14+0] src0_sel:WORD_0 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+12+0], v128 dst_sel:WORD_0 // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+12+0], v129 dst_sel:WORD_1 // Convert to FP16
v_cvt_pk_f32_fp8 v[128:129], v[vgprG2LB+14+0] src0_sel:WORD_1 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+12+1], v128 dst_sel:WORD_0 // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+12+1], v129 dst_sel:WORD_1 // Convert to FP16
v_cvt_pk_f32_fp8 v[128:129], v[vgprG2LB+14+1] src0_sel:WORD_0 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+12+2], v128 dst_sel:WORD_0 // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+12+2], v129 dst_sel:WORD_1 // Convert to FP16
v_cvt_pk_f32_fp8 v[128:129], v[vgprG2LB+14+1] src0_sel:WORD_1 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+12+3], v128 dst_sel:WORD_0 // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+12+3], v129 dst_sel:WORD_1 // Convert to FP16
ds_write_b128 v[vgprLocalWriteAddrB], v[vgprG2LB+12:vgprG2LB+12+3] offset:1728 // lwoB_0_0_3_0 = (0*LSCB)*(MT1J+PAD) + (3*LSPB) = 1728
v_cvt_pk_f32_fp8 v[128:129], v[vgprG2LB+18+0] src0_sel:WORD_0 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+16+0], v128 dst_sel:WORD_0 // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+16+0], v129 dst_sel:WORD_1 // Convert to FP16
v_cvt_pk_f32_fp8 v[128:129], v[vgprG2LB+18+0] src0_sel:WORD_1 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+16+1], v128 dst_sel:WORD_0 // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+16+1], v129 dst_sel:WORD_1 // Convert to FP16
v_cvt_pk_f32_fp8 v[128:129], v[vgprG2LB+18+1] src0_sel:WORD_0 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+16+2], v128 dst_sel:WORD_0 // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+16+2], v129 dst_sel:WORD_1 // Convert to FP16
v_cvt_pk_f32_fp8 v[128:129], v[vgprG2LB+18+1] src0_sel:WORD_1 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+16+3], v128 dst_sel:WORD_0 // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+16+3], v129 dst_sel:WORD_1 // Convert to FP16
ds_write_b128 v[vgprLocalWriteAddrB], v[vgprG2LB+16:vgprG2LB+16+3] offset:2304 // lwoB_0_0_4_0 = (0*LSCB)*(MT1J+PAD) + (4*LSPB) = 2304
v_cvt_pk_f32_fp8 v[128:129], v[vgprG2LB+22+0] src0_sel:WORD_0 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+20+0], v128 dst_sel:WORD_0 // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+20+0], v129 dst_sel:WORD_1 // Convert to FP16
v_cvt_pk_f32_fp8 v[128:129], v[vgprG2LB+22+0] src0_sel:WORD_1 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+20+1], v128 dst_sel:WORD_0 // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+20+1], v129 dst_sel:WORD_1 // Convert to FP16
v_cvt_pk_f32_fp8 v[128:129], v[vgprG2LB+22+1] src0_sel:WORD_0 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+20+2], v128 dst_sel:WORD_0 // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+20+2], v129 dst_sel:WORD_1 // Convert to FP16
v_cvt_pk_f32_fp8 v[128:129], v[vgprG2LB+22+1] src0_sel:WORD_1 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+20+3], v128 dst_sel:WORD_0 // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+20+3], v129 dst_sel:WORD_1 // Convert to FP16
ds_write_b128 v[vgprLocalWriteAddrB], v[vgprG2LB+20:vgprG2LB+20+3] offset:2880 // lwoB_0_0_5_0 = (0*LSCB)*(MT1J+PAD) + (5*LSPB) = 2880
v_cvt_pk_f32_fp8 v[128:129], v[vgprG2LB+26+0] src0_sel:WORD_0 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+24+0], v128 dst_sel:WORD_0 // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+24+0], v129 dst_sel:WORD_1 // Convert to FP16
v_cvt_pk_f32_fp8 v[128:129], v[vgprG2LB+26+0] src0_sel:WORD_1 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+24+1], v128 dst_sel:WORD_0 // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+24+1], v129 dst_sel:WORD_1 // Convert to FP16
v_cvt_pk_f32_fp8 v[128:129], v[vgprG2LB+26+1] src0_sel:WORD_0 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+24+2], v128 dst_sel:WORD_0 // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+24+2], v129 dst_sel:WORD_1 // Convert to FP16
v_cvt_pk_f32_fp8 v[128:129], v[vgprG2LB+26+1] src0_sel:WORD_1 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+24+3], v128 dst_sel:WORD_0 // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+24+3], v129 dst_sel:WORD_1 // Convert to FP16
ds_write_b128 v[vgprLocalWriteAddrB], v[vgprG2LB+24:vgprG2LB+24+3] offset:3456 // lwoB_0_0_6_0 = (0*LSCB)*(MT1J+PAD) + (6*LSPB) = 3456
v_cvt_pk_f32_fp8 v[128:129], v[vgprG2LB+30+0] src0_sel:WORD_0 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+28+0], v128 dst_sel:WORD_0 // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+28+0], v129 dst_sel:WORD_1 // Convert to FP16
v_cvt_pk_f32_fp8 v[128:129], v[vgprG2LB+30+0] src0_sel:WORD_1 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+28+1], v128 dst_sel:WORD_0 // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+28+1], v129 dst_sel:WORD_1 // Convert to FP16
v_cvt_pk_f32_fp8 v[128:129], v[vgprG2LB+30+1] src0_sel:WORD_0 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+28+2], v128 dst_sel:WORD_0 // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+28+2], v129 dst_sel:WORD_1 // Convert to FP16
v_cvt_pk_f32_fp8 v[128:129], v[vgprG2LB+30+1] src0_sel:WORD_1 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+28+3], v128 dst_sel:WORD_0 // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+28+3], v129 dst_sel:WORD_1 // Convert to FP16
ds_write_b128 v[vgprLocalWriteAddrB], v[vgprG2LB+28:vgprG2LB+28+3] offset:4032 // lwoB_0_0_7_0 = (0*LSCB)*(MT1J+PAD) + (7*LSPB) = 4032
v_cvt_pk_f32_fp8 v[128:129], v[vgprG2LB+34+0] src0_sel:WORD_0 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+32+0], v128 dst_sel:WORD_0 // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+32+0], v129 dst_sel:WORD_1 // Convert to FP16
v_cvt_pk_f32_fp8 v[128:129], v[vgprG2LB+34+0] src0_sel:WORD_1 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+32+1], v128 dst_sel:WORD_0 // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+32+1], v129 dst_sel:WORD_1 // Convert to FP16
v_cvt_pk_f32_fp8 v[128:129], v[vgprG2LB+34+1] src0_sel:WORD_0 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+32+2], v128 dst_sel:WORD_0 // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+32+2], v129 dst_sel:WORD_1 // Convert to FP16
v_cvt_pk_f32_fp8 v[128:129], v[vgprG2LB+34+1] src0_sel:WORD_1 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+32+3], v128 dst_sel:WORD_0 // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+32+3], v129 dst_sel:WORD_1 // Convert to FP16
ds_write_b128 v[vgprLocalWriteAddrB], v[vgprG2LB+32:vgprG2LB+32+3] offset:4608 // lwoB_0_0_8_0 = (0*LSCB)*(MT1J+PAD) + (8*LSPB) = 4608
v_cvt_pk_f32_fp8 v[128:129], v[vgprG2LB+38+0] src0_sel:WORD_0 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+36+0], v128 dst_sel:WORD_0 // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+36+0], v129 dst_sel:WORD_1 // Convert to FP16
v_cvt_pk_f32_fp8 v[128:129], v[vgprG2LB+38+0] src0_sel:WORD_1 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+36+1], v128 dst_sel:WORD_0 // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+36+1], v129 dst_sel:WORD_1 // Convert to FP16
v_cvt_pk_f32_fp8 v[128:129], v[vgprG2LB+38+1] src0_sel:WORD_0 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+36+2], v128 dst_sel:WORD_0 // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+36+2], v129 dst_sel:WORD_1 // Convert to FP16
v_cvt_pk_f32_fp8 v[128:129], v[vgprG2LB+38+1] src0_sel:WORD_1 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+36+3], v128 dst_sel:WORD_0 // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+36+3], v129 dst_sel:WORD_1 // Convert to FP16
ds_write_b128 v[vgprLocalWriteAddrB], v[vgprG2LB+36:vgprG2LB+36+3] offset:5184 // lwoB_0_0_9_0 = (0*LSCB)*(MT1J+PAD) + (9*LSPB) = 5184
v_cvt_pk_f32_fp8 v[128:129], v[vgprG2LB+42+0] src0_sel:WORD_0 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+40+0], v128 dst_sel:WORD_0 // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+40+0], v129 dst_sel:WORD_1 // Convert to FP16
v_cvt_pk_f32_fp8 v[128:129], v[vgprG2LB+42+0] src0_sel:WORD_1 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+40+1], v128 dst_sel:WORD_0 // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+40+1], v129 dst_sel:WORD_1 // Convert to FP16
v_cvt_pk_f32_fp8 v[128:129], v[vgprG2LB+42+1] src0_sel:WORD_0 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+40+2], v128 dst_sel:WORD_0 // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+40+2], v129 dst_sel:WORD_1 // Convert to FP16
v_cvt_pk_f32_fp8 v[128:129], v[vgprG2LB+42+1] src0_sel:WORD_1 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+40+3], v128 dst_sel:WORD_0 // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+40+3], v129 dst_sel:WORD_1 // Convert to FP16
ds_write_b128 v[vgprLocalWriteAddrB], v[vgprG2LB+40:vgprG2LB+40+3] offset:5760 // lwoB_0_0_10_0 = (0*LSCB)*(MT1J+PAD) + (10*LSPB) = 5760
v_cvt_pk_f32_fp8 v[128:129], v[vgprG2LB+46+0] src0_sel:WORD_0 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+44+0], v128 dst_sel:WORD_0 // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+44+0], v129 dst_sel:WORD_1 // Convert to FP16
v_cvt_pk_f32_fp8 v[128:129], v[vgprG2LB+46+0] src0_sel:WORD_1 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+44+1], v128 dst_sel:WORD_0 // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+44+1], v129 dst_sel:WORD_1 // Convert to FP16
v_cvt_pk_f32_fp8 v[128:129], v[vgprG2LB+46+1] src0_sel:WORD_0 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+44+2], v128 dst_sel:WORD_0 // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+44+2], v129 dst_sel:WORD_1 // Convert to FP16
v_cvt_pk_f32_fp8 v[128:129], v[vgprG2LB+46+1] src0_sel:WORD_1 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+44+3], v128 dst_sel:WORD_0 // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+44+3], v129 dst_sel:WORD_1 // Convert to FP16
ds_write_b128 v[vgprLocalWriteAddrB], v[vgprG2LB+44:vgprG2LB+44+3] offset:6336 // lwoB_0_0_11_0 = (0*LSCB)*(MT1J+PAD) + (11*LSPB) = 6336
...
/*  mfmaIndex:2  */
ds_read_b128 v[vgprValuB_X2_I0+4:vgprValuB_X2_I0+4+3], v[vgprLocalReadAddrB] offset:9280 // L -> Reg lro=32 swapByteOffset=0 ti=32 vIdx=1 eIdx=0 rIdx=0 oIdx=0 buffer=2 iui=0
ds_read_b128 v[vgprValuB_X2_I0+8:vgprValuB_X2_I0+8+3], v[vgprLocalReadAddrB] offset:18496 // L -> Reg lro=32 swapByteOffset=0 ti=32 vIdx=2 eIdx=0 rIdx=0 oIdx=0 buffer=2 iui=0
s_cselect_b32 s84, s[sgprWrapUA+0], s[sgprGlobalReadIncsA+0] // incLower <- ?
...
/*  mfmaIndex:4  */
ds_read_b128 v[vgprValuB_X4_I0+4:vgprValuB_X4_I0+4+3], v[vgprLocalReadAddrB] offset:9344 // L -> Reg lro=64 swapByteOffset=0 ti=32 vIdx=1 eIdx=0 rIdx=0 oIdx=0 buffer=4 iui=0
s_add_u32 s[sgprSrdA+0], s[sgprSrdA+0], s84        // gra SRD += inc(lower)
...
/*  mfmaIndex:5  */
ds_read_b128 v[vgprValuB_X4_I0+8:vgprValuB_X4_I0+8+3], v[vgprLocalReadAddrB] offset:18560 // L -> Reg lro=64 swapByteOffset=0 ti=32 vIdx=2 eIdx=0 rIdx=0 oIdx=0 buffer=4 iui=0
s_addc_u32 s[sgprSrdA+1], s[sgprSrdA+1], s85       // gra SRD += inc(upper)
v_mfma_f32_16x16x16_f16 acc[8:11], v[vgprValuA_X0_I0+0+2+0:vgprValuA_X0_I0+0+2+0+1], v[vgprValuB_X0_I0+8+2+0:vgprValuB_X0_I0+8+2+0+1], acc[8:11] // left value = acc[8+0:11+0]
/* numPrefetchIter=0 */
...
/*  mfmaIndex:7  */
ds_read_b128 v[vgprValuB_X6_I0+4:vgprValuB_X6_I0+4+3], v[vgprLocalReadAddrB] offset:9408 // L -> Reg lro=96 swapByteOffset=0 ti=32 vIdx=1 eIdx=0 rIdx=0 oIdx=0 buffer=6 iui=0
s_subb_u32 s[sgprShadowLimitA+1], s[sgprShadowLimitA+1], s85 // limit -= inc)
...
/*  mfmaIndex:8  */
ds_read_b128 v[vgprValuB_X6_I0+8:vgprValuB_X6_I0+8+3], v[vgprLocalReadAddrB] offset:18624 // L -> Reg lro=96 swapByteOffset=0 ti=32 vIdx=2 eIdx=0 rIdx=0 oIdx=0 buffer=6 iui=0
s_cmp_eq_u32 s[sgprShadowLimitA+1], 0              // are we within 2^32?
...
/* localReadsVacancy: latencyLeft 2 */
s_cselect_b32 s84, s[sgprWrapUB+0], s[sgprGlobalReadIncsB+0] // incLower <- ?
v_mfma_f32_16x16x16_f16 acc[8:11], v[vgprValuA_X2_I0+0+2+0:vgprValuA_X2_I0+0+2+0+1], v[vgprValuB_X2_I0+8+2+0:vgprValuB_X2_I0+8+2+0+1], acc[8:11] // left value = acc[8+0:11+0]
/* numPrefetchIter=0 */
/* dataAtIterA=0 numReadsIterA=3 skipReadsIterA=2 readsPerIterA=1 */
/* dataAtIterB=0 numReadsIterB=3 skipReadsIterB=2 readsPerIterB=3 */
...
s_waitcnt lgkmcnt(0)
s_barrier
v_mfma_f32_16x16x16_f16 acc[8:11], v[vgprValuA_X4_I0+0+2+0:vgprValuA_X4_I0+0+2+0+1], v[vgprValuB_X4_I0+8+2+0:vgprValuB_X4_I0+8+2+0+1], acc[8:11] // left value = acc[8+0:11+0]
/* numPrefetchIter=0 */
/* dataAtIterA=1 numReadsIterA=3 skipReadsIterA=1 readsPerIterA=1 */
/* dataAtIterB=1 numReadsIterB=3 skipReadsIterB=1 readsPerIterB=3 */
...
s_waitcnt lgkmcnt(15)                              // wait for prior local read local write old=0, new=28 newLW=28 newLR=0
...
ds_read_b128 v[vgprValuB_X0_I0+4:vgprValuB_X0_I0+4+3], v[vgprLocalReadAddrB] offset:9216 // L -> Reg lro=0 swapByteOffset=0 ti=32 vIdx=1 eIdx=0 rIdx=0 oIdx=0 buffer=0 iui=0
ds_read_b128 v[vgprValuB_X0_I0+8:vgprValuB_X0_I0+8+3], v[vgprLocalReadAddrB] offset:18432 // L -> Reg lro=0 swapByteOffset=0 ti=32 vIdx=2 eIdx=0 rIdx=0 oIdx=0 buffer=0 iui=0
v_mfma_f32_16x16x16_f16 acc[8:11], v[vgprValuA_X6_I0+0+2+0:vgprValuA_X6_I0+0+2+0+1], v[vgprValuB_X6_I0+8+2+0:vgprValuB_X6_I0+8+2+0+1], acc[8:11] // left value = acc[8+0:11+0]
/* numPrefetchIter=1 */
...
s_and_b32 s84, 15, s[sgprSizeI]                    // s84 = s[sgprSizeI] % 16
...
s_mov_b32 s87, 0x0                                 // STATIC_DIV: divisior=96
s_mul_i32 s86, 0x555, s[sgprSizeJ]                 // tmp1 = dividend * magic hi
s_lshl_b64 s[86:87], s[86:87], 0x10                // left shift 16 bits
s_mul_i32 s85, s[sgprSizeJ], 0x5556                // tmp0 = dividend * magic lo
s_add_u32 s86, s85, s86                            // add lo
s_addc_u32 s87, s87, 0x0                           // add hi
s_lshr_b64 s[86:87], s[86:87], 0x21                // tmp1 = (dividend * magic) << shift
s_mov_b32 s85, s86                                 // quotient
s_mul_i32 s86, s85, 0x60                           // quotient*divisor
s_sub_u32 s84, s[sgprSizeJ], s86                   // rReg = dividend - quotient*divisor
...
s_and_b32 s85, 127, s[sgprSizesSum+0]              // s85 = s[sgprSizesSum+0] % 128
...
s_mul_i32 s51, 96, s[sgprWorkGroup1]               // wgp1 * MT1
...
s_mul_i32 s60, s[sgprStrideD1J], 64                // scale StrideD *= numRows(32) * bpe
...
s_mul_hi_i32 s85, s84, s[sgprGlobalReadIncsA+0]    // start offset S in bytes
...
s_cselect_b32 s[sgprSrdA+2], s[sgprShadowLimitA+0], BufferLimit // Move shadow to real if we are within 2^32
s_sub_i32 s84, 3, s[sgprStaggerUIter]
s_mul_hi_i32 s85, s84, s[sgprGlobalReadIncsB+0]    // start offset S in bytes
...
buffer_load_ubyte_d16 v[vgprG2LB+34+0], v[vgprGlobalReadOffsetB+0], s[sgprSrdB:sgprSrdB+3], s[sgprScalarGlobalReadOffsetB+7] offen offset:0, sc0 // load one buffer value
/* g2l=32, load component 1 */
buffer_load_ubyte_d16 v0, v[vgprGlobalReadOffsetB+0], s[sgprSrdB:sgprSrdB+3], s[sgprScalarGlobalReadOffsetB+7] offen offset:1, sc0 // load one buffer value
/* g2l=32, load component 2 */
buffer_load_ubyte_d16_hi v1, v[vgprGlobalReadOffsetB+0], s[sgprSrdB:sgprSrdB+3], s[sgprScalarGlobalReadOffsetB+7] offen offset:2, sc0 // load one buffer value
/* g2l=32, load component 3 */
buffer_load_ubyte_d16_hi v2, v[vgprGlobalReadOffsetB+0], s[sgprSrdB:sgprSrdB+3], s[sgprScalarGlobalReadOffsetB+7] offen offset:3, sc0 // load one buffer value
/* g2l=32, load component 4 */
buffer_load_ubyte_d16 v[vgprG2LB+34+1], v[vgprGlobalReadOffsetB+0], s[sgprSrdB:sgprSrdB+3], s[sgprScalarGlobalReadOffsetB+7] offen offset:4, sc0 // load one buffer value
/* g2l=32, load component 5 */
buffer_load_ubyte_d16 v4, v[vgprGlobalReadOffsetB+0], s[sgprSrdB:sgprSrdB+3], s[sgprScalarGlobalReadOffsetB+7] offen offset:5, sc0 // load one buffer value
/* g2l=32, load component 6 */
buffer_load_ubyte_d16_hi v5, v[vgprGlobalReadOffsetB+0], s[sgprSrdB:sgprSrdB+3], s[sgprScalarGlobalReadOffsetB+7] offen offset:6, sc0 // load one buffer value
/* g2l=32, load component 7 */
buffer_load_ubyte_d16_hi v6, v[vgprGlobalReadOffsetB+0], s[sgprSrdB:sgprSrdB+3], s[sgprScalarGlobalReadOffsetB+7] offen offset:7, sc0 // load one buffer value
s_waitcnt vmcnt(6)
v_lshlrev_b32 v0, 0x8, v0                          // shift left to higher 8 bits
v_or_b32 v[vgprG2LB+34+0], v[vgprG2LB+34+0], v0    // pack a sub 8-bit with dest
s_waitcnt vmcnt(5)
v_or_b32 v[vgprG2LB+34+0], v[vgprG2LB+34+0], v1    // pack a sub 8-bit with dest
s_waitcnt vmcnt(4)
v_lshlrev_b32 v2, 0x8, v2                          // shift left to higher 8 bits
v_or_b32 v[vgprG2LB+34+0], v[vgprG2LB+34+0], v2    // pack a sub 8-bit with dest
s_waitcnt vmcnt(2)
v_lshlrev_b32 v4, 0x8, v4                          // shift left to higher 8 bits
v_or_b32 v[vgprG2LB+34+1], v[vgprG2LB+34+1], v4    // pack a sub 8-bit with dest
s_waitcnt vmcnt(1)
v_or_b32 v[vgprG2LB+34+1], v[vgprG2LB+34+1], v5    // pack a sub 8-bit with dest
s_waitcnt vmcnt(0)
v_lshlrev_b32 v6, 0x8, v6                          // shift left to higher 8 bits
v_or_b32 v[vgprG2LB+34+1], v[vgprG2LB+34+1], v6    // pack a sub 8-bit with dest
/* g2l=36, load component 0 */
buffer_load_ubyte_d16 v[vgprG2LB+38+0], v[vgprGlobalReadOffsetB+0], s[sgprSrdB:sgprSrdB+3], s[sgprScalarGlobalReadOffsetB+8] offen offset:0, sc0 // load one buffer value
/* g2l=36, load component 1 */
buffer_load_ubyte_d16 v0, v[vgprGlobalReadOffsetB+0], s[sgprSrdB:sgprSrdB+3], s[sgprScalarGlobalReadOffsetB+8] offen offset:1, sc0 // load one buffer value
/* g2l=36, load component 2 */
buffer_load_ubyte_d16_hi v1, v[vgprGlobalReadOffsetB+0], s[sgprSrdB:sgprSrdB+3], s[sgprScalarGlobalReadOffsetB+8] offen offset:2, sc0 // load one buffer value
/* g2l=36, load component 3 */
buffer_load_ubyte_d16_hi v2, v[vgprGlobalReadOffsetB+0], s[sgprSrdB:sgprSrdB+3], s[sgprScalarGlobalReadOffsetB+8] offen offset:3, sc0 // load one buffer value
/* g2l=36, load component 4 */
buffer_load_ubyte_d16 v[vgprG2LB+38+1], v[vgprGlobalReadOffsetB+0], s[sgprSrdB:sgprSrdB+3], s[sgprScalarGlobalReadOffsetB+8] offen offset:4, sc0 // load one buffer value
/* g2l=36, load component 5 */
buffer_load_ubyte_d16 v4, v[vgprGlobalReadOffsetB+0], s[sgprSrdB:sgprSrdB+3], s[sgprScalarGlobalReadOffsetB+8] offen offset:5, sc0 // load one buffer value
/* g2l=36, load component 6 */
buffer_load_ubyte_d16_hi v5, v[vgprGlobalReadOffsetB+0], s[sgprSrdB:sgprSrdB+3], s[sgprScalarGlobalReadOffsetB+8] offen offset:6, sc0 // load one buffer value
/* g2l=36, load component 7 */
buffer_load_ubyte_d16_hi v6, v[vgprGlobalReadOffsetB+0], s[sgprSrdB:sgprSrdB+3], s[sgprScalarGlobalReadOffsetB+8] offen offset:7, sc0 // load one buffer value
s_waitcnt vmcnt(6)
v_lshlrev_b32 v0, 0x8, v0                          // shift left to higher 8 bits
v_or_b32 v[vgprG2LB+38+0], v[vgprG2LB+38+0], v0    // pack a sub 8-bit with dest
s_waitcnt vmcnt(5)
v_or_b32 v[vgprG2LB+38+0], v[vgprG2LB+38+0], v1    // pack a sub 8-bit with dest
s_waitcnt vmcnt(4)
v_lshlrev_b32 v2, 0x8, v2                          // shift left to higher 8 bits
v_or_b32 v[vgprG2LB+38+0], v[vgprG2LB+38+0], v2    // pack a sub 8-bit with dest
s_waitcnt vmcnt(2)
v_lshlrev_b32 v4, 0x8, v4                          // shift left to higher 8 bits
v_or_b32 v[vgprG2LB+38+1], v[vgprG2LB+38+1], v4    // pack a sub 8-bit with dest
s_waitcnt vmcnt(1)
v_or_b32 v[vgprG2LB+38+1], v[vgprG2LB+38+1], v5    // pack a sub 8-bit with dest
s_waitcnt vmcnt(0)
v_lshlrev_b32 v6, 0x8, v6                          // shift left to higher 8 bits
v_or_b32 v[vgprG2LB+38+1], v[vgprG2LB+38+1], v6    // pack a sub 8-bit with dest
/* g2l=40, load component 0 */
buffer_load_ubyte_d16 v[vgprG2LB+42+0], v[vgprGlobalReadOffsetB+0], s[sgprSrdB:sgprSrdB+3], s[sgprScalarGlobalReadOffsetB+9] offen offset:0, sc0 // load one buffer value
/* g2l=40, load component 1 */
buffer_load_ubyte_d16 v0, v[vgprGlobalReadOffsetB+0], s[sgprSrdB:sgprSrdB+3], s[sgprScalarGlobalReadOffsetB+9] offen offset:1, sc0 // load one buffer value
/* g2l=40, load component 2 */
buffer_load_ubyte_d16_hi v1, v[vgprGlobalReadOffsetB+0], s[sgprSrdB:sgprSrdB+3], s[sgprScalarGlobalReadOffsetB+9] offen offset:2, sc0 // load one buffer value
/* g2l=40, load component 3 */
buffer_load_ubyte_d16_hi v2, v[vgprGlobalReadOffsetB+0], s[sgprSrdB:sgprSrdB+3], s[sgprScalarGlobalReadOffsetB+9] offen offset:3, sc0 // load one buffer value
/* g2l=40, load component 4 */
buffer_load_ubyte_d16 v[vgprG2LB+42+1], v[vgprGlobalReadOffsetB+0], s[sgprSrdB:sgprSrdB+3], s[sgprScalarGlobalReadOffsetB+9] offen offset:4, sc0 // load one buffer value
/* g2l=40, load component 5 */
buffer_load_ubyte_d16 v4, v[vgprGlobalReadOffsetB+0], s[sgprSrdB:sgprSrdB+3], s[sgprScalarGlobalReadOffsetB+9] offen offset:5, sc0 // load one buffer value
/* g2l=40, load component 6 */
buffer_load_ubyte_d16_hi v5, v[vgprGlobalReadOffsetB+0], s[sgprSrdB:sgprSrdB+3], s[sgprScalarGlobalReadOffsetB+9] offen offset:6, sc0 // load one buffer value
/* g2l=40, load component 7 */
buffer_load_ubyte_d16_hi v6, v[vgprGlobalReadOffsetB+0], s[sgprSrdB:sgprSrdB+3], s[sgprScalarGlobalReadOffsetB+9] offen offset:7, sc0 // load one buffer value
s_waitcnt vmcnt(6)
v_lshlrev_b32 v0, 0x8, v0                          // shift left to higher 8 bits
v_or_b32 v[vgprG2LB+42+0], v[vgprG2LB+42+0], v0    // pack a sub 8-bit with dest
s_waitcnt vmcnt(5)
v_or_b32 v[vgprG2LB+42+0], v[vgprG2LB+42+0], v1    // pack a sub 8-bit with dest
s_waitcnt vmcnt(4)
v_lshlrev_b32 v2, 0x8, v2                          // shift left to higher 8 bits
v_or_b32 v[vgprG2LB+42+0], v[vgprG2LB+42+0], v2    // pack a sub 8-bit with dest
s_waitcnt vmcnt(2)
v_lshlrev_b32 v4, 0x8, v4                          // shift left to higher 8 bits
v_or_b32 v[vgprG2LB+42+1], v[vgprG2LB+42+1], v4    // pack a sub 8-bit with dest
s_waitcnt vmcnt(1)
v_or_b32 v[vgprG2LB+42+1], v[vgprG2LB+42+1], v5    // pack a sub 8-bit with dest
s_waitcnt vmcnt(0)
v_lshlrev_b32 v6, 0x8, v6                          // shift left to higher 8 bits
v_or_b32 v[vgprG2LB+42+1], v[vgprG2LB+42+1], v6    // pack a sub 8-bit with dest
/* g2l=44, load component 0 */
buffer_load_ubyte_d16 v[vgprG2LB+46+0], v[vgprGlobalReadOffsetB+0], s[sgprSrdB:sgprSrdB+3], s[sgprScalarGlobalReadOffsetB+10] offen offset:0, sc0 // load one buffer value
/* g2l=44, load component 1 */
buffer_load_ubyte_d16 v0, v[vgprGlobalReadOffsetB+0], s[sgprSrdB:sgprSrdB+3], s[sgprScalarGlobalReadOffsetB+10] offen offset:1, sc0 // load one buffer value
/* g2l=44, load component 2 */
buffer_load_ubyte_d16_hi v1, v[vgprGlobalReadOffsetB+0], s[sgprSrdB:sgprSrdB+3], s[sgprScalarGlobalReadOffsetB+10] offen offset:2, sc0 // load one buffer value
/* g2l=44, load component 3 */
buffer_load_ubyte_d16_hi v2, v[vgprGlobalReadOffsetB+0], s[sgprSrdB:sgprSrdB+3], s[sgprScalarGlobalReadOffsetB+10] offen offset:3, sc0 // load one buffer value
/* g2l=44, load component 4 */
buffer_load_ubyte_d16 v[vgprG2LB+46+1], v[vgprGlobalReadOffsetB+0], s[sgprSrdB:sgprSrdB+3], s[sgprScalarGlobalReadOffsetB+10] offen offset:4, sc0 // load one buffer value
/* g2l=44, load component 5 */
buffer_load_ubyte_d16 v4, v[vgprGlobalReadOffsetB+0], s[sgprSrdB:sgprSrdB+3], s[sgprScalarGlobalReadOffsetB+10] offen offset:5, sc0 // load one buffer value
/* g2l=44, load component 6 */
buffer_load_ubyte_d16_hi v5, v[vgprGlobalReadOffsetB+0], s[sgprSrdB:sgprSrdB+3], s[sgprScalarGlobalReadOffsetB+10] offen offset:6, sc0 // load one buffer value
/* g2l=44, load component 7 */
buffer_load_ubyte_d16_hi v6, v[vgprGlobalReadOffsetB+0], s[sgprSrdB:sgprSrdB+3], s[sgprScalarGlobalReadOffsetB+10] offen offset:7, sc0 // load one buffer value
s_waitcnt vmcnt(6)
v_lshlrev_b32 v0, 0x8, v0                          // shift left to higher 8 bits
v_or_b32 v[vgprG2LB+46+0], v[vgprG2LB+46+0], v0    // pack a sub 8-bit with dest
s_waitcnt vmcnt(5)
v_or_b32 v[vgprG2LB+46+0], v[vgprG2LB+46+0], v1    // pack a sub 8-bit with dest
s_waitcnt vmcnt(4)
v_lshlrev_b32 v2, 0x8, v2                          // shift left to higher 8 bits
v_or_b32 v[vgprG2LB+46+0], v[vgprG2LB+46+0], v2    // pack a sub 8-bit with dest
s_waitcnt vmcnt(2)
v_lshlrev_b32 v4, 0x8, v4                          // shift left to higher 8 bits
v_or_b32 v[vgprG2LB+46+1], v[vgprG2LB+46+1], v4    // pack a sub 8-bit with dest
s_waitcnt vmcnt(1)
v_or_b32 v[vgprG2LB+46+1], v[vgprG2LB+46+1], v5    // pack a sub 8-bit with dest
s_waitcnt vmcnt(0)
v_lshlrev_b32 v6, 0x8, v6                          // shift left to higher 8 bits
v_or_b32 v[vgprG2LB+46+1], v[vgprG2LB+46+1], v6    // pack a sub 8-bit with dest
...
ds_write_b128 v[vgprLocalWriteAddrB], v[vgprG2LB+28:vgprG2LB+28+3] offset:4032 // lwoB_0_0_7_0 = (0*LSCB)*(MT1J+PAD) + (7*LSPB) = 4032
v_cvt_pk_f32_fp8 v[0:1], v[vgprG2LB+34+0] src0_sel:WORD_0 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+32+0], v0 dst_sel:WORD_0  // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+32+0], v1 dst_sel:WORD_1  // Convert to FP16
v_cvt_pk_f32_fp8 v[0:1], v[vgprG2LB+34+0] src0_sel:WORD_1 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+32+1], v0 dst_sel:WORD_0  // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+32+1], v1 dst_sel:WORD_1  // Convert to FP16
v_cvt_pk_f32_fp8 v[0:1], v[vgprG2LB+34+1] src0_sel:WORD_0 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+32+2], v0 dst_sel:WORD_0  // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+32+2], v1 dst_sel:WORD_1  // Convert to FP16
v_cvt_pk_f32_fp8 v[0:1], v[vgprG2LB+34+1] src0_sel:WORD_1 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+32+3], v0 dst_sel:WORD_0  // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+32+3], v1 dst_sel:WORD_1  // Convert to FP16
ds_write_b128 v[vgprLocalWriteAddrB], v[vgprG2LB+32:vgprG2LB+32+3] offset:4608 // lwoB_0_0_8_0 = (0*LSCB)*(MT1J+PAD) + (8*LSPB) = 4608
v_cvt_pk_f32_fp8 v[0:1], v[vgprG2LB+38+0] src0_sel:WORD_0 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+36+0], v0 dst_sel:WORD_0  // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+36+0], v1 dst_sel:WORD_1  // Convert to FP16
v_cvt_pk_f32_fp8 v[0:1], v[vgprG2LB+38+0] src0_sel:WORD_1 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+36+1], v0 dst_sel:WORD_0  // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+36+1], v1 dst_sel:WORD_1  // Convert to FP16
v_cvt_pk_f32_fp8 v[0:1], v[vgprG2LB+38+1] src0_sel:WORD_0 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+36+2], v0 dst_sel:WORD_0  // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+36+2], v1 dst_sel:WORD_1  // Convert to FP16
v_cvt_pk_f32_fp8 v[0:1], v[vgprG2LB+38+1] src0_sel:WORD_1 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+36+3], v0 dst_sel:WORD_0  // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+36+3], v1 dst_sel:WORD_1  // Convert to FP16
ds_write_b128 v[vgprLocalWriteAddrB], v[vgprG2LB+36:vgprG2LB+36+3] offset:5184 // lwoB_0_0_9_0 = (0*LSCB)*(MT1J+PAD) + (9*LSPB) = 5184
v_cvt_pk_f32_fp8 v[0:1], v[vgprG2LB+42+0] src0_sel:WORD_0 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+40+0], v0 dst_sel:WORD_0  // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+40+0], v1 dst_sel:WORD_1  // Convert to FP16
v_cvt_pk_f32_fp8 v[0:1], v[vgprG2LB+42+0] src0_sel:WORD_1 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+40+1], v0 dst_sel:WORD_0  // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+40+1], v1 dst_sel:WORD_1  // Convert to FP16
v_cvt_pk_f32_fp8 v[0:1], v[vgprG2LB+42+1] src0_sel:WORD_0 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+40+2], v0 dst_sel:WORD_0  // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+40+2], v1 dst_sel:WORD_1  // Convert to FP16
v_cvt_pk_f32_fp8 v[0:1], v[vgprG2LB+42+1] src0_sel:WORD_1 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+40+3], v0 dst_sel:WORD_0  // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+40+3], v1 dst_sel:WORD_1  // Convert to FP16
ds_write_b128 v[vgprLocalWriteAddrB], v[vgprG2LB+40:vgprG2LB+40+3] offset:5760 // lwoB_0_0_10_0 = (0*LSCB)*(MT1J+PAD) + (10*LSPB) = 5760
v_cvt_pk_f32_fp8 v[0:1], v[vgprG2LB+46+0] src0_sel:WORD_0 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+44+0], v0 dst_sel:WORD_0  // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+44+0], v1 dst_sel:WORD_1  // Convert to FP16
v_cvt_pk_f32_fp8 v[0:1], v[vgprG2LB+46+0] src0_sel:WORD_1 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+44+1], v0 dst_sel:WORD_0  // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+44+1], v1 dst_sel:WORD_1  // Convert to FP16
v_cvt_pk_f32_fp8 v[0:1], v[vgprG2LB+46+1] src0_sel:WORD_0 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+44+2], v0 dst_sel:WORD_0  // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+44+2], v1 dst_sel:WORD_1  // Convert to FP16
v_cvt_pk_f32_fp8 v[0:1], v[vgprG2LB+46+1] src0_sel:WORD_1 // convert to F32
v_cvt_f16_f32 v[vgprG2LB+44+3], v0 dst_sel:WORD_0  // Convert to FP16
v_cvt_f16_f32 v[vgprG2LB+44+3], v1 dst_sel:WORD_1  // Convert to FP16
ds_write_b128 v[vgprLocalWriteAddrB], v[vgprG2LB+44:vgprG2LB+44+3] offset:6336 // lwoB_0_0_11_0 = (0*LSCB)*(MT1J+PAD) + (11*LSPB) = 6336
...
/* N/A */
v_lshrrev_b32 v0, 7, v[vgprSerial]                 // LSU offset: sgid = Serial / subGroup(128)
s_mov_b32 s5, 96                                   // LSU offset: stride = MT1(96) + PAD1(0)
...
/* Tail: add address/G2L vgpr [64...126) to pool */
...
v_cmp_ge_i32 s[84:85], v64, s[sgprLoopCounterL]    // check K index >= Size L
v_cndmask_b32 v[vgprValuA_X0_I0+0+0], v[vgprValuA_X0_I0+0+0], 0x0, s[84:85] // set 0 if K_idx >= sizeL
v_cndmask_b32 v[vgprValuA_X0_I0+0+1], v[vgprValuA_X0_I0+0+1], 0x0, s[84:85] // set 0 if K_idx >= sizeL
v_cndmask_b32 v[vgprValuB_X0_I0+0+0], v[vgprValuB_X0_I0+0+0], 0x0, s[84:85] // set 0 if K_idx >= sizeL
v_cndmask_b32 v[vgprValuB_X0_I0+2+0], v[vgprValuB_X0_I0+2+0], 0x0, s[84:85] // set 0 if K_idx >= sizeL
v_cndmask_b32 v[vgprValuB_X0_I0+4+0], v[vgprValuB_X0_I0+4+0], 0x0, s[84:85] // set 0 if K_idx >= sizeL
v_cndmask_b32 v[vgprValuB_X0_I0+0+1], v[vgprValuB_X0_I0+0+1], 0x0, s[84:85] // set 0 if K_idx >= sizeL
v_cndmask_b32 v[vgprValuB_X0_I0+2+1], v[vgprValuB_X0_I0+2+1], 0x0, s[84:85] // set 0 if K_idx >= sizeL
v_cndmask_b32 v[vgprValuB_X0_I0+4+1], v[vgprValuB_X0_I0+4+1], 0x0, s[84:85] // set 0 if K_idx >= sizeL
s_nop 1
v_mfma_f32_16x16x16_f16 acc[0:3], v[vgprValuA_X0_I0+0+0+0:vgprValuA_X0_I0+0+0+0+1], v[vgprValuB_X0_I0+0+0+0:vgprValuB_X0_I0+0+0+0+1], acc[0:3] // left value = acc[0+0:3+0]
...
/* Tail: remove address/G2L [64...126) from pool */
label_Summation_End_3QZT0RV47W21AY7N_0:
/* endSummation: add vgpr [0...126) to pool */
...
s_waitcnt lgkmcnt(0)                               // wait for 16 bytes of kern args.
s_cmp_eq_u32 s[sgprGSU], 1                         // GSU == 1 ?
s_cbranch_scc0 label_NoBranch_31U1IQZZEJ1V1BI6_0   // Only branch on scc1
...
label_NoBranch_31U1IQZZEJ1V1BI6_0:
...
s_mul_i32 s60, s[sgprStrideD1J], 128               // scale StrideD *= numRows(32) * bpe
s_add_u32 s[sgprSrdD+0], s[sgprSrdD+0], s60        // incToNextRow: gra SRD += inc(lower)
s_addc_u32 s[sgprSrdD+1], s[sgprSrdD+1], 0         // incToNextRow: gra SRD += inc(upper)
...
s_mul_i32 s63, s63, 2                              // cal a batch offset
...
s_mov_b32 s70, s[sgprSrdD+2]                       // Move HELLO
s_mov_b32 s71, s[sgprSrdD+3]                       // Move HELLO
s_mul_i32 s66, s[sgprStrideD1J], 128               // scale StrideD *= numRows(32) * bpe
...
//bps 8
//rpv 2.0
s_mul_i32 s60, s[sgprStrideD1J], 64                // scale StrideTD *= numRows(32) * bpe
...
/* (d1,vc1,d0,vc0)=(1,0,0,0) */
v_add_co_u32 v1, vcc, v1, 32                       // coord1.1: coord1Vgpr += d1*sg1*VW + vc1
...
s_mul_i32 s67, s67, 2                              // cal a batch offset
...
/* (d1,vc1,d0,vc0)=(1,0,0,0) */
s_mul_i32 s60, s[sgprStrideC1J], 64                // scale StrideC *= numRows(32) * bpe
